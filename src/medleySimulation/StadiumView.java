...
public class StadiumView extends JPanel implements Runnable {
	...
	public void paintComponent(Graphics g) {
		...
		for (i = 0; i < maxX; i += 5) { // columns
			...
			lane++;
			g.drawLine((i + xBorder) * wIncr, hIncr * yBorder, (i + xBorder) * wIncr, (endPool + yBorder) * hIncr); // - leave
																																																							// space
																																																							// at
																																																							// bottom
			g.setColor(Color.white);
			g.fillRect((i + 2 + xBorder) * wIncr, (endPool + yBorder) * hIncr, wIncr, hIncr); // draw starting blocks outside
																																												// pool
		}
		g.setColor(laneColours[lane]);
		g.drawLine((i + xBorder) * wIncr, hIncr * yBorder, (i + xBorder) * wIncr, (endPool + yBorder) * hIncr); // draw last
																																																						// one
