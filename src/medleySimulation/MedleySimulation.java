...
public class MedleySimulation {
	...
	static CountDownLatch latch = new CountDownLatch(1); // Latch to prevent the program from running before pressing the
																												// start button
	...
	public static void setupGUI(int frameX, int frameY) {
		...
		counterDisplay = new CounterDisplay(winner, finishLine); // to update score
		...
		startB.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				// Release the latch to start the simulation
				latch.countDown();
			}
		});
	...
	public static void main(String[] args) throws InterruptedException {
		...
		peopleLocations = new PeopleLocation[numTeams * SwimTeam.sizeOfTeam]; // 4 swimmers per team
		...
		// prevent the program from running before pressing the start button
		latch.await();
